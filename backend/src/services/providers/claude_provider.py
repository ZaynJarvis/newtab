"""Claude (Anthropic) provider for LLM services."""
...
class ClaudeProvider(BaseLLMProvider):
    """Claude provider for LLM services. Note: Claude doesn't provide embeddings."""
    
    def __init__(self, api_key: str, base_url: str = "https://api.anthropic.com/v1", 
                 llm_model: str = "claude-3-sonnet-20240229", **kwargs):
        super().__init__(api_key, **kwargs)
        self.base_url = base_url.rstrip('/')
        self.llm_model = llm_model
        self.llm_endpoint = f"{self.base_url}/messages"
    ...
    async def generate_keywords_and_description(self, title: str, content: str) -> Dict[str, str]:
        """Generate keywords, description, and improved title using Claude."""
...
{{
    ...
    "improved_title": "Better title if original is generic, or original title if already good"
        ...
        try:
            ...
            self.logger.error(
                "Error generating keywords/description with Claude",
                extra={
                    "error": str(e),
                    "title": title[:100],
...
class ClaudeEmbeddingProvider:
    """
    Placeholder class for Claude embeddings.
    
    Note: Anthropic Claude does not provide embedding services.
    This class exists for consistency but will raise NotImplementedError.
    Use OpenAI or another provider for embeddings when using Claude for LLM.
    """
    
    def __init__(self, *args, **kwargs):
        raise NotImplementedError(
            "Claude does not provide embedding services. "
            "Use OpenAI, Groq, or another provider for embeddings."
        )
